...
Windowsにおけるキーボードの入力は以下の準に行われる。

1. ハードウェア（キーボード：HID）
1. Kernel空間
   1. `kbdhid.sys`（HID→スキャンコード変換）
   1. `kbdclass.sys`（クラスドライバ）
   1. キーボードレイアウト変換
      1. `Scancode Map`によるスキャンコード変換
1. User空間
   1. `Win32`入力サブシステム
      1. 仮想キー(VK)の状態管理
   1. フック
      1. PowerToyのような低レベルフック
   1. 対象のアプリケーション設定

レジストリ変更による`Scancode Map`の変更は、Kernel空間での処理を変更する。  
つまり、OSがどのキーが来たかの認識を誤認させている。  

対して、Powertoyの場合は、OSの認識はCAPSLOCKのままでアプリケーションに届けるコードを変更している。

### Caps Lockが特殊な理由

Caps Lockキーは、「押す→離す」の単純なキーではなく、トグル状態を持つ特殊なキーである。  
そして、物理スキャンコードがCapsLock(`0x3A`)でOSが認識した場合、WindowsはVK_CAPITALをトグルキーとして扱う。  
内部状態（GetKeyState(VK_CAPITAL)のトグルビット）とキーボードのLEDを同期させる。

つまり、OSにCapsLockキーコードが処理された段階で、CapsLock特有のトグル処理はOS側で行われる。  
たとえ、それ以下のユーザーランドでいくらキーコードを変えたとしても、OSはCapsLockとして認識しているので押下状態を作ってしまうのだ。
